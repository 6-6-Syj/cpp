...
/*   Updated: 2026/05/27 20:59:14 by jmagand          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PmergeMe<T>::fordJohnsonSort(T &chain)
{
	...
	// Récursion sur la liste des grands (main)
	...
	// RE-SYNCHRONISATION : On reconstruit pend et pairs basés sur l'ordre final de main
	T pend;
	T synchronizedPairs;
	for (size_t i = 0; i < main.size(); ++i)
	{
	...
	// Phase d'insertion binaire avec limites strictes
...
{
	T jacobsthalValues = jacobsthalSequence(pendSize);
	T insertionOrder;
	std::vector<bool> used(pendSize, false);

	std::cout << MAGENTA << "\n[GenInsertOrder] Début de la construction (pendSize = " << pendSize << "):\n"
			  << RESET;
	...
	for (size_t i = 0; i < jacobsthalValues.size(); i++)
	{
		...
		std::cout << MAGENTA << "  Bloc Jacobsthal J=" << jacobsthalValues[i] << " -> Indices ajoutés : [ " << RESET;
	...
	for (int i = 0; i < pendSize; i++)
	{
		if (!used[i])
		{
			if (!hasLeftovers)
			{
				std::cout << MAGENTA << "  Reste des indices -> Indices ajoutés : [ " << RESET;
				hasLeftovers = true;
			}
